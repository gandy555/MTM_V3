...
BOOL CGpioMtm::Init()
{
	...
	// 출력 (초기상태 확인요!)
#if 0
	SetDir(GPIOG, 0, GPIO_OUT);	//LCD BackLight	(Low=Off, High=On)
	SetPin(GPIOG, 0, HIGH);

	SetDir(GPIOG, 1, GPIO_OUT);	//LED (Low=Off, High=On)
	SetPin(GPIOG, 1, HIGH);

	SetDir(GPIOG, 2, GPIO_OUT);	//RS485 Control (Low=Off, High=On)
//	SetPin(GPIOG, 2, LOW);
	SetPin(GPIOG, 2, HIGH);

	SetDir(GPIOG, 7, GPIO_OUT);	//Relay (Low=Off, High2014-01-21=On)
	SetPin(GPIOG, 7, LOW);
...
void CGpioMtm::CheckGpioState(UINT gpio_enum, GPIO_GROUP group, int num, USHORT* pusGpioFlag)
{
	...
	//	*pusGpioFlag |= (1 << gpio_enum);
		SET_FLAG(*pusGpioFlag, FLAG(gpio_enum));
	else
	//	*pusGpioFlag &= ~(1 << gpio_enum);
		CLR_FLAG(*pusGpioFlag, FLAG(gpio_enum));
}

#ifndef __WALLPAD_PROTO_SHN_H__
#define __WALLPAD_PROTO_SHN_H__
...
// Header
//
#define SHN_HDR_WALLPAD					0xCD	//월패드 --> 생활정보기
#define SHN_HDR_DEV						0xB0	//생활정보기 --> 월패드

//
// Data(Command)
//
#define SHN_CMD_WEATHER_RES				0x01	//월패드에서 생활정보기로 날씨정보 전송
#define SHN_CMD_WEATHER_REQ				0x02	//생활정보기에서 월패드로 날씨정보 요청

#define SHN_CMD_PARKING_RES				0x03	//월패드에서 생활정보기로 주차정보 전송
#define SHN_CMD_PARKING_REQ				0x04	//생할정보기에서 월패드로 주차정보 요청

#define SHN_CMD_ELEVATOR_STAT_REQ		0x05	//생할정보기에서 월패드로 엘레베이터 상태요청
#define SHN_CMD_ELEVATOR_STAT_RES		0x06	//월패드에서 생활정보기로 엘레베이터 상태전송
#define SHN_CMD_ELEVATOR_CALL			0x07	//생할정보기에서 월패드로 엘레베이터 호출

#define SHN_CMD_WALLPAD_STAT_REQ		0x08	//생할정보기에서 월패드로 일괄소등,가스,외출설정 상태요청

#define SHN_CMD_LAMP_ONOFF_REQ			0x09	//생할정보기에서 월패드로 일괄소등 On/Off요청

#define SHN_CMD_GAS_OFF_REQ				0x0A	//생할정보기에서 월패드로 가스밸브 Off요청

#define SHN_CMD_OUT_ONOFF_REQ			0x0B	//생할정보기에서 월패드로 외출설정 On/Off요청
#define SHN_CMD_OUT_STAT_RES			0x0C	//월패드에서 생활정보기로 외출설정 상태전송

#define SHN_CMD_GAS_STAT_RES			0x0D	//월패드에서 생활정보기로 가스밸브 상태전송

#define SHN_CMD_ELEVATOR_ARRIVE			0x0E	//월패드에서 생활정보기로 엘리베이터 목적층 도착시 메세지 전송

#define SHN_CMD_INIT_DATA				0x5A	//월패드에서 생활정보기로 : 월패드가 켜질때, 월패드가 Ack Data 10회이상 미수신할 경우

#define SHN_CMD_FREE_DATA				0x41	//월패드에서 생활정보기로 : Event Data 없을 경우, Live Check
...
/*
	...
	날씨정보 요청 (0x02) : 기기-->월패드
		...
		CD 02 01 00 CS		
*/
typedef struct _SHN_WEATHER_INFO
{
	UCHAR		weather_icon;				//날씨 아이콘 (1~40)
	UCHAR		temp_high;					//최고기온
	UCHAR		temp_low;					//최저기온
} __attribute__ ((packed)) SHN_WEATHER_INFO;

typedef struct _SHN_WEATHER_RES
{
	UCHAR				year;				//년
	UCHAR				month;				//월
	UCHAR				day;				//일
	...
	SHN_WEATHER_INFO	weather_info[5];	//오늘~4일후
} __attribute__ ((packed)) SHN_WEATHER_RES;	//size=22
...
typedef struct _SHN_PARKING_RES
{
	UCHAR			year;					//년
	UCHAR			month;					//월
	UCHAR			day;					//일
...
} __attribute__ ((packed)) SHN_PARKING_RES;	//size=15
...
/*
	...
	엘레베이터 상태응답 (0x06) : 기기<--월패드
		...
		CD 06 03 xx xx xx CS
									B0 06 01 00 CS
*/
typedef struct _SHN_ELEVATOR_STATUS
{
	UCHAR			status;					//Status
	UCHAR			floor;					//현재층
	UCHAR			dir;					//방향
} __attribute__ ((packed)) SHN_ELEVATOR_STATUS;	//size=3
...
/*
	엘레베이터 호출 (0x07) : 기기-->월패드
		...
		CD 07 02 xx xx CS
									B0 41 01 00 CS
*/
typedef struct _SHN_ELEVATOR_CALL_RES
{
	UCHAR			state;					//방향(상태)
	UCHAR			status;					//Status
} __attribute__ ((packed)) SHN_ELEVATOR_CALL_RES;	//size=2
...
/*
	월패드 상태요청 (0x08) : 기기-->월패드
		...
		CD 08 03 xx xx xx CS
									B0 41 01 00 CS
*/
typedef struct _SHN_WALLPAD_STATUS
{
	UCHAR			alloff_stat;			//일괄소등상태
	UCHAR			gas_stat;				//가스밸브상태
	UCHAR			security_stat;			//외출설정상태
} __attribute__ ((packed)) SHN_WALLPAD_STATUS;	//size=3

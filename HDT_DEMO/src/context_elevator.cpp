...
static void simulate_elevator(void)
{
	UCHAR buffer_mtm[MTM_PACKET_MAX] = {0,};
	PMTM_HEADER pMtmHdr = (PMTM_HEADER)&buffer_mtm[0];
	PMTM_DATA_ELEVATOR	pMtmData = (PMTM_DATA_ELEVATOR)&buffer_mtm[sizeof(MTM_HEADER)];
	UCHAR ev_status;
	static char ev_floor = 1;
	...
	pMtmData->status = g_setup_data.m_SetupData.elevator_status;
	pMtmData->floor  = g_setup_data.m_SetupData.elevator_floor;
...
#if defined(CONFIG_USE_SHOWCASE)
			g_timer.SetTimer(ELEVATOR_TIMER, 2, NULL, "ELEVATOR SIMULATION TIMER");
#endif
		}
		break;
	case 1:
